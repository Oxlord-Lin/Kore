...
def defend_dangerous_shipyard_agent(obs, config):
    ...
    for shipyard in shipyards:
        for opp_shipyard in opp_shipyards:
            dist = opp_shipyard.position.distance_to(shipyard.position, size)
            if dist <= 2: # 建在我家门口，这能忍？
                dangerous_shipyards.append(opp_shipyard)

    if len(dangerous_shipyards) > 0: # 如果有高威胁性船厂
        dangerous_shipyards = list(sorted(dangerous_shipyards, key = lambda shipyard : shipyard.max_spawn))
        target_opp_shipyard :Shipyard= dangerous_shipyards[0] # 进攻目标


        shipyards = sample(shipyards, len(shipyards))
        for shipyard in shipyards:
            flight_plan = get_shortest_flight_path_between(shipyard.position, target_opp_shipyard.position, size)
            
            if shipyard.position.distance_to(target_opp_shipyard.position,size) <= 2: # 如果目标船厂就在门口
                if shipyard.ship_count > 50:
                    shipyard.next_action = ShipyardAction.launch_fleet_with_flight_plan(50, flight_plan)
                else: 
                    shipyard.next_action = ShipyardAction.launch_fleet_with_flight_plan(min(shipyard.ship_count,25), flight_plan)
            
            if shipyard.position.distance_to(target_opp_shipyard.position,size) <= 10: # 如果目标船厂在适合进攻的范围内，则派出的船队随距离递减
                dist = shipyard.position.distance_to(target_opp_shipyard.position,size) 
                shipyard.next_action = ShipyardAction.launch_fleet_with_flight_plan(min(shipyard.ship_count,120//dist), flight_plan)

                                
            # 如果目标攻击船厂不在合适的进攻范围内，则转入defenser策略
    ...
    ## 如果不存在高威胁性船厂，则转入defend_dangerous_fleet策略
    else:
