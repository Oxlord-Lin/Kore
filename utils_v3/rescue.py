...
def rescue_agent(obs, config):
            ...
            if direction % 2 == 0: # 如果沿南-北方向飞行
                ...
                for shipyard in shipyards:
                    if shipyard.position.y == y:
                        ...
                        break
                if homeless:
                    homeless_my_fleet.append(fleet)
    ...
    if len(homeless_my_fleet) > 0 and len(shipyards) > 0:
        homeless_my_fleet = list(sorted(homeless_my_fleet,key = lambda fleet: fleet.kore))
        fleet_to_rescue :Fleet = homeless_my_fleet[-1] # 携带最多矿石的fleet最值得营救

        fleet_size = fleet_to_rescue.ship_count
        
        shipyards = list(sorted(shipyards,key = lambda shipyard: shipyard.ship_count))
        max_ship_shipyard = shipyards[-1]

        if max_ship_shipyard.ship_count > fleet_size + 30: # 如果适合营救
            mag_x = 1 if fleet_to_rescue.position.x > max_ship_shipyard.position.x else -1
            abs_x = abs(fleet_to_rescue.position.x - max_ship_shipyard.position.x)
            dir_x = mag_x if abs_x < size/2 else -mag_x
            mag_y = 1 if fleet_to_rescue.position.y > max_ship_shipyard.position.y else -1
            abs_y = abs(fleet_to_rescue.position.y - max_ship_shipyard.position.y)
            ...
            if fleet_to_rescue.direction.to_char() == 'N':
                flight_plan = flight_path_x + 'S9N9'
                flight_plan += 'W' if dir_x == 1 else 'E'
            
            elif fleet_to_rescue.direction.to_char() == 'S':
                flight_plan = flight_path_x + 'N9S9'
                flight_plan += 'W' if dir_x == 1 else 'E'

            elif fleet_to_rescue.direction.to_char() == 'W':
                flight_plan = flight_path_y + 'E9W9'
                flight_plan += 'S' if dir_y == 1 else 'N'

            elif fleet_to_rescue.direction.to_char() == 'E':
